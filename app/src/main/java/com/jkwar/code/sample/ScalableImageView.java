...
public class ScalableImageView extends View implements NestedScrollingChild2 {
  ...
  @Override public boolean startNestedScroll(int i, int i1) {
    return childHelper.startNestedScroll(i, i1);
  }

  @Override public void stopNestedScroll(int i) {
    childHelper.stopNestedScroll(i);
  }

  @Override public boolean hasNestedScrollingParent(int i) {
  ...
  class HenGestureListener extends GestureDetector.SimpleOnGestureListener {
    ...
    public boolean onScroll(MotionEvent down, MotionEvent event, float distanceX, float distanceY) {
      if (big) {
        offsetX -= distanceX;
        offsetY -= distanceY;
        int dyConsumed = fixOffsets();
        if (dyConsumed != 0) {
          childHelper.dispatchNestedScroll(0, 0, 0, dyConsumed, null);
        } else {
          invalidate();
        }
      }
  ...
  private int fixOffsets() {
    ...
    int result = 0;
    //往上拉不能小于 -(bitmap.getHeight() * bigScale - getHeight()) / 2
    //往下拉不能大于 (bitmap.getHeight() * bigScale - getHeight()) / 2
